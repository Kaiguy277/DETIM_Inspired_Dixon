"""
Bayesian ensemble calibration of Dixon Glacier DETIM — v13 (CAL-013, D-028).

Multi-objective calibration with snowline elevation in the MCMC likelihood,
plus post-hoc area evolution behavioral filter.

Changes from CAL-012 (v12):
  - Snowline elevation chi-squared terms added to MCMC log-likelihood
    (~19 years of digitized snowlines, sigma=75m)
  - Post-MCMC behavioral filter on glacier area evolution
    (6 checkpoints 2000-2025, manually digitized outlines)
  - Same 6 free parameters, same priors, same DE+MCMC structure
...
def main(resume=False):
    ...
    if resume and (OUTPUT_DIR / 'de_multistart_v13.json').exists():
        print(f"\n{'=' * 70}")
        print(f"PHASE 1: SKIPPED (loading saved DE results)")
        print(f"{'=' * 70}")
        with open(OUTPUT_DIR / 'de_multistart_v13.json') as f:
        ...
        # Save all DE logs
        pd.DataFrame(all_de_logs).to_csv(OUTPUT_DIR / 'calibration_log_v13_de.csv', index=False)
        ...
        with open(OUTPUT_DIR / 'de_multistart_v13.json', 'w') as f:
            json.dump(de_summary, f, indent=2)

        # Save best overall params
        best_mode = modes[0]
        with open(OUTPUT_DIR / 'best_params_v13.json', 'w') as f:
    ...
    for mode_idx, mode in enumerate(modes):
        ...
        chain_file = OUTPUT_DIR / f'mcmc_chain_v13_mode{mode_id}.npy'
        lp_file = OUTPUT_DIR / f'mcmc_logprob_v13_mode{mode_id}.npy'
        prev_chain = None
        prev_lp = None
        steps_done = 0
    ...
    with open(OUTPUT_DIR / 'posterior_param_names_v13.json', 'w') as f:
    ...
    posterior_df.to_csv(OUTPUT_DIR / 'posterior_samples_v13.csv', index=False)
    ...
    try:
        ...
        fig.savefig(OUTPUT_DIR / 'corner_plot_v13.png', dpi=150, bbox_inches='tight')
        print(f"\n  Corner plot saved to {OUTPUT_DIR / 'corner_plot_v13.png'}")
    ...
    chain_files = sorted(OUTPUT_DIR.glob('mcmc_chain_v13_mode*.npy'))
    lp_files = sorted(OUTPUT_DIR.glob('mcmc_logprob_v13_mode*.npy'))
    ...
    pd.DataFrame(area_scores).to_csv(
        OUTPUT_DIR / 'area_filter_v13_scores.csv', index=False)
    ...
    with open(OUTPUT_DIR / 'filtered_params_v13.json', 'w') as f:
        json.dump(filtered_data, f, indent=2)
    print(f"  Filtered params saved: filtered_params_v13.json ({n_survived} sets)")
    ...
    with open(OUTPUT_DIR / 'calibration_summary_v13.json', 'w') as f:
    ...
    print(f"    python run_projection.py --filtered-params "
          f"calibration_output/filtered_params_v13.json")
    print("Done!")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', action='store_true',
                        help='Skip DE phase, load saved optima from de_multistart_v13.json')
